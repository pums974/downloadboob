...
def check_backend(backend_name):
    ...
    output, err = Popen(['videoob', "backend", "list"],
                        stdout=PIPE,
    ...
    if backend_name in list_backend:
        ...
        output, err = Popen(["videoob", "backend", "list-modules"],
                            stdout=PIPE,
                            stderr=PIPE).communicate()
        logging.debug(output)
        if err:
...
def check_exec(executable):
    try:
        output, err = Popen(['which', executable],
                            stdout=PIPE,
                            stderr=PIPE).communicate()
        if err:
...
class Downloadboob(object):
    def __init__(self, name, backend_name, download_directory,
                 links_directory):
    ...
    def is_downloaded(self, video):
        if os.path.isfile(self.get_filename(video)) or os.path.isfile(
    ...
    def is_ok(self, video, title_regexp, id_regexp, author_regexp,
              title_exclude):
    ...
    def get_linkname(self, video, m3u=False):
        ...
        if not kodi:
            ...
            if not misc:
                misc = video.id
            return "%s/%s (%s).%s" % (self.links_directory, removeSpecial(
                video.title), removeSpecial(misc), ext)
        else:
    ...
    def videoob_list_rep(self, rep, backend):
        list_id = []
        list_rep = list(self.backend.iter_resources(
            (BaseVideo, ), rep.split("/")))
        for fich in list_rep:
            if fich.id:
                list_id.append(fich.id)
            else:
                list_id.append(self.videoob_list_rep(rep + "/" + fich.title,
                                                     backend))
        return sorted(set(list_id))

    def videoob_get_info(self, video):  # THIS IS COSTLY
        try:
            output, err = Popen(['videoob', "info", "--backend=" +
                                 self.backend.name, "--", video.id],
                                stdout=PIPE,
                                stderr=PIPE).communicate()
            ...
            sys.stderr = stderr
            try:
                self.backend.fill_video(video, (
                    'ext', 'title', 'url', 'duration', 'author', 'date',
                    'description'))  # This is incomplete for some backends
    ...
    def do_search(self,
        ...
        if pattern_type == "search":
            ...
            for id in self.videoob_list_rep(pattern, self.backend.name):
                sys.path.insert(0, backend_directory + "/" + self.backend.name
                                + "/")  # HACK
    ...
    def filter_list(self, list_videos, title_regexp, id_regexp, author_regexp,
        ...
        for video in list_videos:
            if self.is_ok(video, title_regexp, id_regexp, author_regexp,
                          title_exclude):
                if not self.is_downloaded(video):
                    ...
                    if not video:
                        ...
                        if self.is_ok(video, title_regexp, id_regexp,
                                      author_regexp, title_exclude):
                            i += 1
                            if not self.is_downloaded(video):
                                videos.append(video)
                                logging.info(
                                    ...
                                    % (video.title, video.description,
                                       video.author, video.id, video.duration,
                                       video.date))
                            if i == max_results:
                                break
        return videos
        ...
        if videos:
            for video in videos:
                ...
                if not ret:
                    if not kodi:
                        self.set_link(
                            video)  # CREATE LINKS FOR A BEAUTIFULL LIBRARY
                    else:
        ...
        while True:
            try:
                ...
                if check_backend(backend_name):
                    ...
                    section_sublinks_directory = config.get(
                        section, "directory").decode('utf8')
                    if config.has_option(section, "type"):
                        pattern_type = config.get(section,
                                                  "type").decode('utf8')
                    else:
                        pattern_type = "search"
                    if config.has_option(section, "title_regexp"):
                        title_regexp = config.get(
                            section, "title_regexp").decode('utf8')
                    else:
                        title_regexp = None
                    if config.has_option(section, "title_exclude"):
                        title_exclude = config.get(
                            section, "title_exclude").decode('utf8')
                    else:
                        title_exclude = None
                    if config.has_option(section, "id_regexp"):
                        id_regexp = config.get(section,
                                               "id_regexp").decode('utf8')
                    else:
                        id_regexp = None
                    if config.has_option(section, "author_regexp"):
                        author_regexp = config.get(
                            section, "author_regexp").decode('utf8')
                    else:
                        author_regexp = None
                    if config.has_option(section, "max_results"):
                        ...
                        max_result = 50
                    section_links_directory = os.path.join(
                        links_directory, section_sublinks_directory)
                    #       if not backend_name == "youtube":

                    # Initialize and do some cleaning
                    downloadboob = Downloadboob(section, backend_name,
                                                download_directory,
                    ...
                    downloadboob.download(pattern, CapVideo.SEARCH_DATE, False,
                                          max_result, title_regexp, id_regexp,
                                          pattern_type, author_regexp,
                                          title_exclude)
                    ...
                    if pattern_type == "search":  # FIXME (AT LEAST) FOR YOUTUBE
                        logging.info(
                            "For backend %s, start search-bis for '%s'" %
                            (backend_name, section))
                        downloadboob.download(
                            pattern, CapVideo.SEARCH_RELEVANCE, False,
                            max_result, title_regexp, id_regexp, pattern_type,
                            author_regexp, title_exclude)
...
config.read(['/etc/downloadboob.conf',
             os.path.expanduser('~/downloadboob.conf'), 'downloadboob.conf'])
...
if config.has_option("main", "backend_directory"):
    backend_directory = os.path.expanduser(
        config.get("main", "backend_directory").decode('utf8'))
else:
...
if __name__ == '__main__':
    ...
    processes = [Process(target=do_work,
                         args=(work_queue, res_queue, )) for i in range(nproc)]
    # Let's work
    for p in processes:
        p.start()
